...
const packages = [
  {
    ...
    days: 3,
    price: 3,
    features: ["ظهور أعلى السوق", "شارة 🔥 مروّج", "زيادة فرص البيع"],
  },
  {
    id: "7days",
    title: "ترويج 7 أيام",
    days: 7,
    price: 5,
    features: ["ظهور أعلى السوق", "شارة 🔥 مروّج", "أفضلية داخل القسم"],
  },
  {
    id: "30days",
    title: "ترويج 30 يوم",
    days: 30,
    price: 10,
    features: ["ظهور قوي", "شارة 🔥 مروّج", "أفضلية طويلة المدة"],
  },
];
...
export default function BoostRequestPage() {
  ...
  const submitRequest = async () => {
    ...
    try {
      ...
      alert("تم إرسال طلب الترويج. سيتم تفعيله بعد مراجعة الدفع.");
      router.push("/my-posts");
    } catch (error) {
      console.error(error);
    ...
    <main className="min-h-screen bg-slate-100 p-4 md:p-6">
      <div className="mx-auto max-w-5xl space-y-6">
        <section className="rounded-3xl bg-slate-950 p-6 text-white shadow-xl">
          <h1 className="text-3xl font-black">🔥 طلب ترويج الإعلان</h1>
        ...
        <section className="rounded-3xl bg-white p-6 shadow">
          <h2 className="mb-2 text-xl font-black">الإعلان</h2>
          <p className="text-lg font-bold text-slate-900">
            {post?.title || "بدون عنوان"}
          </p>
          <p className="mt-1 text-sm text-slate-500">
            السعر: {post?.price ? `$${post.price}` : "حسب الاتفاق"}
          </p>
        </section>
        ...
        <section className="rounded-3xl bg-white p-6 shadow">
          <h2 className="mb-4 text-xl font-black">طريقة الدفع</h2>
          ...
          <div className="mb-4 rounded-2xl bg-orange-50 p-4 text-sm leading-7 text-orange-900">
            <p className="font-black">تعليمات الدفع:</p>
            <p>
              بعد اختيار الباقة، أرسل المبلغ عبر الطريقة المختارة ثم اكتب رقم
              العملية أو أي ملاحظة في الحقل التالي.
            </p>
